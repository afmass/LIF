...
jobs <- read_delim(
  # file.path(here::here(), "data/ign_jobs.csv"),
  "data/ign_jobs.csv",
  delim = ";",
  trim_ws = TRUE,
...
jobs_nancy <- jobs |>
  mutate(
    ...
    publish_date = dmy(publish_date),
    expiry_date  = dmy(expiry_date)
  ) |>
  filter(
...
render_job <- function(j) {
  ...
  header_line <- paste(pieces, collapse = " · ")
